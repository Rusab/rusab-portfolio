  ...
  return (
    <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50">
      <motion.div
        ...
        className="flex items-center gap-2 px-4 py-3 bg-black/40 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl shadow-blue-900/20"
      >
        {navItems.map((item) => (
          <div key={item.name} className="relative group">
            <button
              onClick={() => handleNavClick(item.href)}
              className={`p-3 rounded-xl transition-all duration-300 ${(pathname === item.href || (item.href.startsWith('/#') && pathname === '/'))
